...
def _google_table(df: pd.DataFrame, group_col: str, label: str,
    ...
    """產出 Google 深度頁的標準表格(8 欄,CPI 用 progress bar 突顯)。"""
    g = df.groupby(group_col).agg(
        spend=("spend", "sum"),
        installs=("installs", "sum"),
        clicks=("clicks", "sum"),
    ...
    # CPI 的最大值給 progress bar 用(越大條越長 = 越貴 = 越紅警示)
    cpi_max = disp["CPI"].max() if disp["CPI"].max() > 0 else 1
    ...
    st.dataframe(
        ...
        height=460,
        column_config={
            "花費($)": st.column_config.NumberColumn("花費($)", format="$%.0f"),
            ...
            "安裝": st.column_config.NumberColumn("安裝", format="%d"),
            "CPI": st.column_config.ProgressColumn(
                "💎 CPI($)",
                help="每次安裝成本(主要觀察指標,條越長 = 越貴)",
                format="$%.2f",
                min_value=0.0,
