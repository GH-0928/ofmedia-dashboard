...
def _google_table(df: pd.DataFrame, group_col: str, label: str,
    ...
    """產出 Google 深度頁的標準表格(8 欄,CPI 用熱力背景突顯)。"""
    g = df.groupby(group_col).agg(
        spend=("spend", "sum"),
        installs=("installs", "sum"),
        clicks=("clicks", "sum"),
    ...
    # 顯示欄位:花費 / 曝光 / CPM / 點擊 / CTR / 安裝 / CPI / CVR
    disp = g[[group_col, "spend", "impressions", "CPM($)", "clicks",
              "CTR(%)", "installs", "CPI($)", "CVR(%)"]].copy()
    disp.columns = [label, "花費($)", "曝光", "CPM($)", "點擊",
                     "CTR(%)", "安裝", "💎 CPI($)", "CVR(%)"]

    # CPI 用熱力背景:綠 = 便宜的安裝(好),紅 = 貴的安裝(差)
    # 只對有安裝的列上色,沒安裝(CPI=0)不算
    cpi_for_grad = disp["💎 CPI($)"].replace(0, float("nan"))
    vmin = cpi_for_grad.min() if not cpi_for_grad.isna().all() else 0
    vmax = cpi_for_grad.max() if not cpi_for_grad.isna().all() else 1

    styled = (
        disp.style
        .background_gradient(
            subset=["💎 CPI($)"], cmap="RdYlGn_r",
            vmin=vmin, vmax=vmax,
        )
        .format({
            "花費($)": "${:,.0f}",
