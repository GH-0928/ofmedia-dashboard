...
| 分頁 | 內容 |
|---|---|
| 投放總覽 | 八個 KPI（規模四個、效率四個，含對比期變化與近 7 天走勢）、異常警示、每日趨勢（含廣告操作標記）、**單日變化歸因**、媒體分布、期間內操作紀錄 |
...
某天安裝暴增或花費驟減時，用來回答「是哪個媒體、哪個 Campaign 造成的」。

- **點每日趨勢圖上的任一天**（或用下方的日期下拉）→ 顯示當天 vs 前 7 天日均的差異，
  再拆成媒體層與 Campaign 層，欄位「貢獻度」是該項目的變化佔整體變化的比例
- **貢獻度可以是負的**（該項目往反方向動，抵銷了整體變化），長條會轉成紅色
- **趨勢圖可切換「依媒體」堆疊**，不點也能看出某天是誰撐起來的
- **異常警示會自動列出單日偏離**：當天總量偏離前 7 天日均超過門檻（預設 15%，可改 20% / 30%），
  直接寫出貢獻最大的媒體與其 Campaign。另外要求安裝變化滿 100 個、花費變化滿 $500，
  避免「40 個安裝掉到 33」這種大百分比小數字的雜訊；最多列 5 條，其餘收成註腳

基準用**前 7 天日均**而不是前一天：前一天本身可能就是異常值，拿它當基準會互相抵銷。
...
| `analysis.py` | 單日歸因的計算（純資料，不碰畫面，可直接測試）|
